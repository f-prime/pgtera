...
fn get_render_path() -> String {
    if let Ok(Some(r)) = Spi::get_one::<String>("select path from pgtera.render_path order by id desc") {
        r
    } else {
...
fn pgtera_set_render_path(file_path: &'static str) {
    let query = format!("insert into pgtera.render_path (path) values ($FP${}$FP$)", file_path);
...
fn pgtera_render(template_name: &'static str, ctx: String) -> String {
    let v: Vec<CtxInput>= serde_json::from_str(ctx.as_str()).expect("Context type is invalid.");
    ...
    let rendered = tera.render(template_name, &context).expect("Error while rendering template.");
